...
class Detective extends Conmmon
{
    ...
    public function index()
    {
        ...
        # 关卡配置
        $passInfo = Db::table('Detective_config_passInfo')->field('pass_id,pic,small_pic,description,analysis,tips,answer,prize')->select();
        $passInfoArr = dataGroup($passInfo, 'pass_id');
        foreach ($passInfoArr as $kk => $vv) {
            foreach ($vv as $key => $value) {
                if (empty($value['answer']) || empty($value['prize'])) continue;

//            $passInfoArr[$kk][$key]['pic'] = $value['pic'];
        ...
        # 探员配置
        $roleArray = Db::table('Detective_config_role')->field('role_id,role_lv,role_unlock_coin,role_up_coin,role_output_coin')->select();
        $data['Role'] = dataGroup($roleArray, 'role_id');

        # 家具配置
        $roleArray = Db::table('Detective_config_furniture')->field('furniture_id,furniture_lv,furniture_unlock_coin,furniture_up_coin,furniture_output_vit')->select();
        $data['Furniture'] = dataGroup($roleArray, 'furniture_id');

//      # 签到列表
//      $signList = Db::table('Detective_signlist')->field('day,num,type,status')->select();
...
//        }
//
//        $res = Db::table('Detective_challenge')->field('status')->where(['user_id' => $this->user_id, 'timestamp' => $post['timestamp']])->find();
//        if ($res) {
//            if ($res['status'] == 0) {
//                $updata = [
//                    'status' => $post['status'],
...
//                ];
//                Db::table('Detective_challenge')->where(['user_id' => $this->user_id, 'timestamp' => $post['timestamp']])->update($updata);
...
//                'end_time' => date('Y-m-d H:i:s'),
//            ];
//            Db::table('Detective_challenge')->insert($insert);
...
//        $user_id = 1;
    ...
    {
        $roleArray = Db::table('Detective_config_role')->field('role_id,role_lv,role_unlock_coin,role_up_coin,role_output_coin')->select();
        $data['roleArray'] = dataGroup($roleArray, 'role_id');
        return jsonResult('', 200, $roleArray);
    }
