...
class Motorcycles extends Conmmon
{

    public function cronTab_rank()
    {
        $insert = Db::Table('motorcycles_user')->field('user_id,user_name,avatar,max_pass,now() as add_time')->where('login_date', date('Y-m-d'))->order('max_pass DESC')->limit(30)->select();
        $rank = Db::Table('motorcycles_rank')->select();
//        dump($insert);die;

        if (empty($rank)) {
            Db::Table('motorcycles_rank')->insertAll($insert);
        } else {
            foreach ($insert as $k => $v) {
                Db::Table('motorcycles_rank')->where(['rank_id' => $k + 1])->update($v);
            }
        }
    }
    ...
    public function getcarCode()
    {
        if (request()->isPost()) {
            ...
            $data = $this->get_motorcycles_openid($code); // 获得 openid 和 session_key
            $user = Db::Table('motorcycles_user')->field('user_id,openid')->where(['openid' => $data['openid']])->find();
            $num = substr(time(), -6);
            ...
            if (empty($user)) {
                ...
                $uid = Db::Table('motorcycles_user')->insertGetId($arr);
                $res['mstr'] = lock_url($data['openid'] . ',' . $uid);
                return jsonResult('请求成功', 200, $res);

            } else {
//                Db::Table('motorcycles_user')->where(['user_id' => $user['user_id']])->update(['end_time' => date('Y-m-d H::s')]);
    ...
    public function getUserInfo()
    {
        ...
        $user = Db::Table('motorcycles_user')->where(['user_id' => $this->user_id])->value('user_id');
        ...
        Db::Table('motorcycles_user')->where(['user_id' => $this->user_id])->update($updata);
    ...
    public function index()
    {
        ...
        # 用户信息
        $userInfo = Db::Table('motorcycles_user')
            ->field('user_id,user_name,avatar,city,max_pass,coin,is_impower,is_car,car,is_role,role,add_time')
            ->where(['user_id' => $this->user_id])
            ->find();
        $userInfo['is_car'] = empty($userInfo['is_car']) ? [0, 0] : json_decode($userInfo['is_car']);
//        $userInfo['is_newplayer'] = 0;
...
//        }
        ...
        // 剩余看视频的次数
        $video_num = Db::table('motorcycles_watch_video')->where(['type' => 10, 'user_id' => $this->user_id, 'add_time' => date('Y-m-d')])->count();
        $userInfo['video_num'] = (5 - $video_num) < 0 ? 0 : 5 - $video_num;

        // 角色商城
        $roleArr = Db::Table('motorcycles_garages_role')->select();
        ...
        // 车库
        $res = Db::Table('motorcycles_garages_class')->where(['is_show' => 1])->select();
        $arr = Db::Table('motorcycles_garages')->select();
        foreach ($res as $k => $v) {
            foreach ($arr as $i => $j) {
...
//            dump($car);die;
        ...
        # 金币配置
        $data['coin_config'] = config('motor_coin_config1');

        # 匹配
        $data['match_car'] = Db::Table('motorcycles_garages')->select();

        # 地图配置
        $map_config = config('motor_map_config2');
        ...
        # 根据IP地址是否开启误点
        $config = Db::table('motorcycles_config')->find();
...
//            dump($car);die;
        ...
        # 金币配置
        $data['coin_config'] = config('motor_coin_config1213');

        # 匹配
        $data['match_car'] = Db::Table('motorcycles_garages')->select();

        # 地图配置
        $map_config = config('motor_map_config1213');
        foreach ($map_config as $k => $v) {
            $map_config[$k]['px'] = count($v['mapArr']) * 2000;
        }
        $data['map_config'] = $map_config;
    ...
    {
        $ranks = Db::Table('motorcycles_rank')->field('user_id,user_name,avatar,max_pass')->select();
        $userIds = array_column($ranks, 'user_id');
        $is_rank = array_keys($userIds, $this->user_id);

        $user = Db::Table('motorcycles_user')->field('user_id,user_name,avatar,max_pass')->where(['user_id' => $this->user_id])->find();
        if (empty($is_rank)) {
            if (empty($ranking)) {
//                $ranking = Db::Table('motorcycles_user')->where(['max_pass' => ['>', $user['max_pass']]])->count();
    ...
    # 离线
    public function offline() // 1.0.9  11-29
    {
...
//        }
//
//        $res = Db::Table('motorcycles_challenge')->field('status')->where(['user_id' => $this->user_id, 'timestamp' => $post['timestamp']])->find();
//        if ($res) {
//            if ($res['status'] == 0) {
//                $updata = [
//                    'status' => $post['status'],
//                    'end_time' => date('Y-m-d H:i:s'),
//                ];
//                Db::Table('motorcycles_challenge')->where(['user_id' => $this->user_id, 'timestamp' => $post['timestamp']])->update($updata);
...
//                'end_time' => date('Y-m-d H:i:s'),
//            ];
//            Db::Table('motorcycles_challenge')->insert($insert);
//            return jsonResult('点击游戏开始', 200);
//        }
    }

    # 渠道统计
    public function statistics_channel_old()
    {
        $user_id = $this->user_id;
...
//        $date = date('Y-m-d');
        ...
        $is_channel = Db::Table('motorcycles_user')->field('user_id,channel,enter_id')->where(['user_id' => $this->user_id])->find();
        if (empty(intval($is_channel['channel']))) { // 用户表channel为空
            ...
            Db::Table('motorcycles_user')->where(['user_id' => $user_id])->update($upData);
...
//        $date_time = '2019-12-06 00:00:00';
        if ($date_time >= '2019-12-07 00:00:00') {
            ...
            $user = Db::Table('motorcycles_user')->field('user_id,channel,login_date')->where(['user_id' => $this->user_id])->find();
...
//        $user_id = 1;
        ...
        if (strlen($app_id) > 6) {
            $app_id = Db::Table('motorcycles_app')->where(['app_id' => $app_id, 'play_status' => 0])->value('id');
        }

        $is_click = Db::Table('motorcycles_statistics_adv')->where(['user_id' => $this->user_id, 'app_id' => $post['app_id'], 'status' => 1, 'add_time' => $date])->value('statistics_id');
        ...
        if (empty($is_click)) {
            ...
            Db::Table('motorcycles_statistics_adv')->insert($advData);

            if ($post['status'] == 1) {
                # 广告限量
                $restrict_id = Db::Table('motorcycles_restrict')->where(['app_id' => $app_id, 'add_time' => $date])->value('restrict_id');
                if (empty($restrict_id)) {
                    ...
                    Db::Table('motorcycles_restrict')->insert($addData);
                } else {
                    Db::Table('motorcycles_restrict')->where(['restrict_id' => $restrict_id])->setInc('num');
                }
            }
    ...
    public function watchVideo()
    {
        ...
        $post['timestamp'] = time();
        Db::Table('motorcycles_watch_video')->insert($post);
...
//            'add_time' => date('Y-m-d H:i:s'),
//        ];
//        Db::Table('motorcycles_record')->insert($insert);
