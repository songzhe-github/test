...
class Qgalaxide extends Conmmon
{
    /**
    ...
    public function getCode()
    {
        if (request()->isPost()) {
            ...
            $user = $db->table('QQ_galaxide_user')->field('user_id,openid,status,new_player_status')->where(['openid' => $data['openid']])->find();
            if (empty($user)) {
                $arr = [
                    'openid' => $data['openid'],
                    'user_name' => '',
                ...
                $uid = $db->table('QQ_galaxide_user')->insertGetId($arr);
                $res['mstr'] = lock_url($data['openid'] . ',' . $uid);
                return jsonResult('请求成功', 200, $res);

            } else {
                $db->table('QQ_galaxide_user')->where(['user_id' => $user['user_id']])->update(['end_time' => date('Y-m-d H::s')]);
    ...
    public function getUserInfo()
    {
        ...
        $db = Db::connect('multi-platform');
        $user = $db->table('QQ_galaxide_user')->where(['user_id' => $this->user_id])->value('user_id');
        ...
        $db->table('QQ_galaxide_user')->where(['user_id' => $this->user_id])->update($updata);
    ...
    public function home()
    {
        ...
        $db = Db::connect('multi-platform');
        # 首页用户信息
        $user = $db->table('QQ_galaxide_user')
        ...
        # 用户的机甲
        $shop = $db->table('QQ_galaxide_shop')->select();
        ...
        # 金币配置
        $data['free_coin'] = $db->table('QQ_galaxide_free_coin')->select();

        # 关卡配置
        $pass = $db->table('QQ_galaxide_pass2')->select();
        $data['pass'] = $pass;

        # 开关配置 （渠道进入才有误点）
        $config = $db->table('QQ_galaxide_config')->field('isCanErrorClick,record_status,version_number')->find();
        ...
        # 角色配置
        $role = $db->table('QQ_galaxide_role')
        ...
        $data['role'] = $role;
        $data['skill_stk'] = 0.5;
        $db->table('QQ_galaxide_user')->where(['user_id' => $user['user_id']])->update(['end_time' => date('Y-m-d H::s'), 'get_free_coin_time' => time()]);
    ...
    {
        $db = Db::connect('multi-platform');
        $ranks = $db->table('QQ_galaxide_rank')->select();
        $userIds = array_column($ranks, 'user_id');
        $is_rank = array_keys($userIds, $this->user_id);

        $user = $db->table('QQ_galaxide_user')->field('user_id,user_name,avatar,max_pass')->where(['user_id' => $this->user_id])->find();
        if (empty($is_rank)) {
            if (empty($ranking)) {
//                $ranking = $db->table('QQ_galaxide_user')->where(['max_pass' => ['>', $user['max_pass']]])->count();
    ...
    public function signList()
    {
        $date = date('Y-m-d');
        $db = Db::connect('multi-platform');
        $signList = $db->table('QQ_galaxide_signlist')->field('day,sign_num,type,status')->select();
        $userList = $db->table('QQ_galaxide_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();

        $end = end($userList);
        if ($end['add_time'] < $date && count($userList) >= 7) {
            # 满一周status更新为0
            $db->table('QQ_galaxide_sign')->where(['user_id' => $this->user_id])->update(['status' => 1]);
            $userList = [];
        }
    ...
    public function sign()
    {
        $date = date('Y-m-d');
        $db = Db::connect('multi-platform');
        $user_sign = $db->table('QQ_galaxide_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();
        $end = end($user_sign);
        ...
        $db->table('QQ_galaxide_sign')->insert($insert);
    ...
    {
        $role_id = Db::table('QQ_galaxide_user')->where(['user_id' => $this->user_id])->value('role_id');
    ...
    public function challenge()
    {
        ...
        $db = Db::connect('multi-platform');
        $max_pass = $db->table('QQ_galaxide_user')->where(['user_id' => $this->user_id])->value('max_pass');
        if ($post['pass'] > $max_pass) {
            $db->table('QQ_galaxide_user')->where(['user_id' => $this->user_id])->update(['max_pass' => $post['pass']]);
        }
        ...
        $db->table('QQ_galaxide_challenge')->insert($insert);
...
//        dump($post);die;
        Db::table('QQ_galaxide_user')->where(['user_id' => $this->user_id])->update($post);
    ...
    public function offline_new()
    {
        ...
        Db::table('QQ_galaxide_user')->where(['user_id' => $this->user_id])->update($userInfo);
        return jsonResult('离线记录成功', 200);
    }
    ...
    public function adverts()
    {
        $date = date('Y-m-d');
        $db = Db::connect('multi-platform');
        $click_today = $db->table('QQ_galaxide_statistics_adv')->where(['user_id' => $this->user_id, 'status' => 1, 'add_time' => $date])->column('app_id');
        $click = $db->table('QQ_galaxide_statistics_adv')->where(['user_id' => $this->user_id, 'status' => 1])->column('app_id');

        # 点击过和限量的广告ID
        $restrict = $db->table('QQ_galaxide_app')
            ->alias('app')
            ->join(['QQ_galaxide_restrict ' => 'r'], 'app.id=r.app_id and r.num >= app.restrict')
...
//            ->limit(8)
        ...
        # banner广告
        $banner = $db->table('QQ_galaxide_app')
    ...
    public function more()
    {
        $date = date('Y-m-d');
        $db = Db::connect('multi-platform');
        $click = $db->table('QQ_galaxide_statistics_adv')->where(['user_id' => $this->user_id, 'status' => 1, 'add_time' => $date])->column('app_id');
        # 点击过和限量的广告ID
        $restrict = $db->table('QQ_galaxide_app')
            ->alias('app')
            ->join(['QQ_galaxide_restrict ' => 'r'], 'app.id=r.app_id and r.num >= app.restrict')
...
//            ->limit(8)
        ...
        if ($banner_num < 5) {
            ...
            $banner1 = $db->table('QQ_galaxide_app')
                ->field('id,app_id,app_name,app_text,app_qrcode_url,app_banner_url,page')
                ->where(['id' => ['NOT IN', $banner_not], 'banner_status' => 0])
                ->order('RAND()')
                ->limit($limit)
...
//        $qd_id = input('post.qd_id'); //前端传过来的渠道ID
        ...
        $db = Db::connect('multi-platform');
        $is_channel = $db->table('QQ_galaxide_user')->field('user_id,channel,enter_id')->where(['user_id' => $this->user_id])->find();

//        dump($is_channel);die;
        if (empty(intval($is_channel['channel']))) { // 用户表channel为空

            if ($is_channel['enter_id']) { // 用户表有enter_id channel为0
                $channel = $db->table('QQ_galaxide_channel')->where(['enter_id' => $is_channel['enter_id']])->value('qd_id');
                $upData = [
                    'channel' => $channel,
                    'end_time' => date('Y-m-d H:i:s'),
                ];
                ...
                $channel2 = $db->table('QQ_galaxide_channel')->where(['enter_id' => $enter_id])->value('qd_id');
                if ($channel2) {
                    $upData = [
                        'channel' => $channel2,
                        'scene' => $post['scene'],
...
//            dump($upData);die;
            $db->table('QQ_galaxide_user')->where(['user_id' => $user_id])->update($upData);
    ...
    public function statistics_channel()
    {
        ...
        $db = Db::connect('multi-platform');
        $record_channel_id = $db->table('QQ_galaxide_record_channel')->where(['user_id' => $this->user_id, 'add_date' => $date])->value('record_channel_id');
        if (empty($record_channel_id)) {
            ...
            $db->table('QQ_galaxide_record_channel')->insert($record);
    ...
    public function statistics_adv()
    {
        ...
        if ($post['win_status'] == 1) {
            $text = '成功';
        } else if ($post['win_status'] == 2) {
            $text = '失败';
        }

        $db = Db::connect('multi-platform');
        if (strlen($app_id) > 6) {
            $app_id = $db->table('QQ_galaxide_app')->where(['app_id' => $app_id])->value('id');
        }

        $is_click = $db->table('QQ_galaxide_statistics_adv')->where(['user_id' => $this->user_id, 'app_id' => $post['app_id'], 'status' => 1, 'add_time' => $date])->value('statistics_id');
        ...
        if (empty($is_click)) {
            ...
            $db->table('QQ_galaxide_statistics_adv')->insert($advData);

            if ($post['status'] == 1) {
                # 广告限量
                $restrict_id = $db->table('QQ_galaxide_restrict')->where(['app_id' => $app_id, 'add_time' => $date])->value('restrict_id');
                if (empty($restrict_id)) {
                    ...
                    $db->table('QQ_galaxide_restrict')->insert($addData);
                } else {
                    $db->table('QQ_galaxide_restrict')->where(['restrict_id' => $restrict_id])->setInc('num');
                }
            }
    ...
    public function record()
    {
        ...
        Db::table('QQ_galaxide_record')->insert($data);
    ...
    public function watchVideo()
    {
        ...
        $post['timestamp'] = time();
        Db::table('QQ_galaxide_watch_video')->insert($post);
        return jsonResult('记录成功', 200);
    }
