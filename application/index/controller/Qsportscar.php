...
class Qsportscar extends Conmmon
{
    /**
     * 获取code，返回openid
     */
    public function getcarCode()
    {
        if (request()->isPost()) {
            ...
            $appid = '1110496532';
            $appsecret = 'uaMlawExIRfhZTan';
            $data = $this->getQQOpenid($appid, $appsecret, $code); // 获得 openid 和 session_key


            $user = Db::Table('QQ_sportsCar_user')->field('user_id,openid')->where(['openid' => $data['openid']])->find();
            if (empty($user)) {
                $arr = [
                    'openid' => $data['openid'],
                    'user_name' => '游客' . time(),
                ...
                $uid = Db::Table('QQ_sportsCar_user')->insertGetId($arr);
                $res['mstr'] = lock_url($data['openid'] . ',' . $uid);
                return jsonResult('新用户', 200, $res);

            } else {
//                Db::Table('QQ_sportsCar_user')->where(['user_id' => $user['user_id']])->update(['end_time' => date('Y-m-d H::s')]);
    ...
    public function getUserInfo()
    {
        ...
        $user = Db::Table('QQ_sportsCar_user')->where(['user_id' => $this->user_id])->value('user_id');
        ...
        Db::Table('QQ_sportsCar_user')->where(['user_id' => $this->user_id])->update($updata);
    ...
    public function index()
    {
        ...
        # 用户信息
        $userInfo = Db::Table('QQ_sportsCar_user')
        ...
        // 剩余看视频的次数
        $video_num = Db::Table('QQ_sportsCar_watch_video')->where(['type' => 10, 'user_id' => $this->user_id, 'add_time' => date('Y-m-d')])->count();
        ...
        // 车库
        $res = Db::Table('QQ_sportsCar_garages_class')->where(['is_show' => 1])->select();
        $arr = Db::Table('QQ_sportsCar_garages')->select();
        foreach ($res as $k => $v) {
            foreach ($arr as $i => $j) {
...
//            dump($car);die;
        ...
        # 今日榜单
        $rank_one = Db::table('QQ_sportsCar_rank_one')->where('user_name', $userInfo['user_name'])->find();
        $is_rank = 0;
        if ($rank_one && $rank_one['status'] == 0) {
            Db::table('QQ_sportsCar_rank_one')->where('user_name', $userInfo['user_name'])->update(['status' => 1]);
            $is_rank = 1;
        }
        $rankArr = [
            'is_rank' => $is_rank,
        ...
        # 三日榜单
        $rank_three = Db::table('QQ_sportsCar_rank_three')->where('user_name', $userInfo['user_name'])->find();
        $is_rank = 0;
        if ($rank_three && $rank_three['status'] == 0) {
            Db::table('QQ_sportsCar_rank_three')->where('user_name', $userInfo['user_name'])->update(['status' => 1]);
            $is_rank = 1;
        }
        $rankArr1 = [
            'is_rank' => $is_rank,
        ...
        # 匹配
        $data['match_car'] = Db::Table('QQ_sportsCar_garages')->select();

        # 地图配置
        $map_config = config('QQ_singlecar_map_config_bak');
        ...
        # 根据IP地址是否开启误点
        $config = Db::Table('QQ_sportsCar_config')->find();
        ...
        # 转盘列表
        $data['prizeList'] = Db::table('QQ_sportsCar_prizeList')->select();

        # 签到列表
        $signList = Db::table('QQ_sportsCar_signlist')->field('day,num,type,status')->select();
        $sign = Db::table('QQ_sportsCar_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();
        $end = end($sign);
        if ($end['add_time'] < $this->date && count($sign) >= 5) {
            # 满一周status更新为0
            Db::table('QQ_sportsCar_sign')->where(['user_id' => $this->user_id])->update(['status' => 1]);
            $sign = [];
        }
    ...
    {
        $user_name = Db::table('QQ_sportsCar_user')->where('user_id', $this->user_id)->value('user_name');
        # 单日
        $ranks = Db::Table('QQ_sportsCar_rank_one')->field('user_name,avatar,max_pass')->select();
        $userIds = array_column($ranks, 'user_name');
        $is_rank = array_keys($userIds, $user_name);

        $user = Db::Table('QQ_sportsCar_user')->field('user_name,max_pass')->where('user_id', $this->user_id)->find();
        if (empty($is_rank)) {
            $ranking1 = '99+';
        } else {
        ...
        # 三日
        $ranks_three = Db::Table('QQ_sportsCar_rank_three')->field('user_name,avatar,max_pass')->select();
        $userIds = array_column($ranks, 'user_name');
        $is_rank = array_keys($userIds, $user_name);
...
//        dump($upData);
    ...
    public function challenge_bak()
    {
        ...
        Db::Table('QQ_sportsCar_challenge')->insert($insert);
    ...
    public function challenge()
    {
        ...
        $max_distance = Db::table('QQ_sportsCar_user')->where('user_id', $this->user_id)->value('max_distance');
        if ($post['max_pass'] > $max_distance) {
            Db::table('QQ_sportsCar_user')->where('user_id', $this->user_id)->update(['max_distance' => $post['max_pass']]);
        }

        $one_length = $redis->zCard('QQ_SportsCarRank_one');
        # 一日
        $one_score = $redis->zScore('QQ_SportsCarRank_one', $post['user_name']);
        if ($one_score) {
            if ($post['max_pass'] > $one_score) {
                $redis->zAdd('QQ_SportsCarRank_one', $post['max_pass'], $post['user_name']);
            }
        } else {
            if ($one_length < 30) {
                $redis->zAdd('QQ_SportsCarRank_one', $post['max_pass'], $post['user_name']);
                ...
                // 返回分数从低到高排序的前10名及分数
                $revRange = $redis->zRange('QQ_SportsCarRank_one', 0, 0, true);
                $key = array_keys($revRange);
                $value = array_values($revRange);

                if ((int)$post['max_pass'] > (int)$value[0]) {
                    // 删除成员
                    $redis->zRem('QQ_SportsCarRank_one', $key[0]);
                    $redis->zAdd('QQ_SportsCarRank_one', $post['max_pass'], $post['user_name']);
                }
            }
        }

        $three_length = $redis->zCard('QQ_SportsCarRank_three');
        # 三日
        $three_score = $redis->zScore('QQ_SportsCarRank_three', $post['user_name']);
        if ($three_score) {
            if ($post['max_pass'] > $three_score) {
                $redis->zAdd('QQ_SportsCarRank_three', $post['max_pass'], $post['user_name']);
            }
        } else {
            if ($three_length < 30) {
                $redis->zAdd('QQ_SportsCarRank_three', $post['max_pass'], $post['user_name']);
                ...
                // 返回分数从高到低排序的前10名及分数
                $revRange = $redis->zRange('QQ_SportsCarRank_three', 0, 0, true);
                $key = array_keys($revRange);
                $value = array_values($revRange);

                if ((int)$post['max_pass'] > (int)$value[0]) {
                    // 删除成员
                    $redis->zRem('QQ_SportsCarRank_three', $key[0]);
                    $redis->zAdd('QQ_SportsCarRank_three', $post['max_pass'], $post['user_name']);
                }
            }
        }

        $rankArr = $redis->zRevRange('QQ_SportsCarRank_one', 0, 4, true);
        $ranks = [];
        ...
        $count = $redis->zRevRange('QQ_SportsCarRank_one', $one_length - 1, $one_length - 1, true);
        $five = $redis->zRevRange('QQ_SportsCarRank_one', 0, 4, true);
        $start = array_values($five)[count(array_values($five)) - 1];
        $end = array_values($count)[count(array_values($count)) - 1];
        if ($post['max_pass'] >= $start) {
            $ranking = $redis->zRevRank('QQ_SportsCarRank_one', $post['user_name']) + 1; // 从高到低排序的名次
    ...
    public function statistics_channel()
    {
        ...
        $record_channel_id = Db::Table('QQ_sportsCar_record_channel')->where(['user_id' => $this->user_id, 'add_date' => $date])->value('record_channel_id');
        if (empty($record_channel_id)) {
            ...
            Db::Table('QQ_sportsCar_record_channel')->insert($record);
    ...
    public function watchVideo()
    {
        ...
        if ($post['type'] == 16) {
            ...
            Db::table('QQ_sportsCar_prize')->insert($insert);
        }
        $post['user_id'] = $this->user_id;
        $post['add_time'] = date('Y-m-d');
        $post['timestamp'] = time();
        Db::Table('QQ_sportsCar_watch_video')->insert($post);
    ...
    public function freeAcceleration()
    {
        ...
        Db::Table('QQ_sportsCar_free_acceleration')->insert($insert);
    ...
    {
        $user_sign = Db::table('QQ_sportsCar_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();
        $end = end($user_sign);
        ...
        Db::table('QQ_sportsCar_sign')->insert($insert);
    ...
    {
        $prizeNum = Db::table('QQ_sportsCar_prize')->where(['user_id' => $this->user_id, 'add_date' => $this->date])->count('id');
        if ($prizeNum >= 10) {
            return jsonResult('今日抽奖次数已达上限', 100);
        }
        $prize_arr = Db::table('QQ_sportsCar_prizeList')->field('prizeList_id,gl,num,type')->select();
        $hb = Db::table('QQ_sportsCar_prize')->where(['user_id' => $this->user_id, 'prize_id' => 3])->value('id');
        ...
        if ($prize_arr[$rid]['type'] == 3) {
            Db::table('QQ_sportsCar_user')->where('user_id', $this->user_id)->setInc('money', floatval($prize_arr[$rid]['num']));
        }
        $insert = [
            'user_id' => $this->user_id,
            'prize_id' => $rid,
    ...
    public function drawActivity()
    {
        ...
        $activityList = Db::table('QQ_sportsCar_activity')->where(['user_id' => $this->user_id, 'activityList_id' => $activityList_id])->find();
        ...
        Db::table('QQ_sportsCar_activity')->insert($insert);

        $prize = Db::table('QQ_sportsCar_activityList')->where(['activityList_id' => $activityList_id])->value('prize');
        Db::table('QQ_sportsCar_user')->where('user_id', $this->user_id)->setInc('money', $prize);
    ...
    {
        $activityList = Db::table('QQ_sportsCar_activityList')->select();
        $activity = Db::table('QQ_sportsCar_activity')->where('user_id', $this->user_id)->field('activityList_id')->select();
        ...
        for ($i = 1; $i <= 5; $i++) {
            if ($i == 1) {
                $res['max_distance'] = Db::table('QQ_sportsCar_user')->where('user_id', $this->user_id)->value('max_distance');
            } else if ($i == 2) {
                $res['prize'] = Db::table('QQ_sportsCar_prize')->where('user_id', $this->user_id)->count('id');
            } else if ($i == 3) {
                $res['sign'] = Db::table('QQ_sportsCar_sign')->where(['user_id' => $this->user_id])->count('sign_id');
            } else if ($i == 4) {
                $res['shareMessage'] = Db::table('QQ_sportsCar_shareMessage')->where(['user_id' => $this->user_id])->count('id');
            } else if ($i == 5) {
                $res['watchVideo'] = Db::table('QQ_sportsCar_watch_video')->where(['user_id' => $this->user_id, 'add_time' => ['>', '2020-04-30'], 'type' => 2])->count('watch_id');
            }
        }
...
//        dump(lock_url('aaaaaaaaaaaaaaa,11111111'));die;
        ...
        $share_id = $id[1];
        $is_newplayer = Db::table('QQ_sportsCar_user')->where(['user_id' => $this->user_id])->value('car');
        if (empty($is_newplayer)) {
            $share = Db::table('QQ_sportsCar_shareMessage')->where('share_id', $this->user_id)->find();
            if (empty($share)) {
                $insert = [
                    'user_id' => $share_id,
                ...
                Db::table('QQ_sportsCar_shareMessage')->insert($insert);
