...
class Snipe extends Conmmon
{
    ...
    public function getCode()
    {
        if (request()->isPost()) {
            ...
            $appid = 'wxde7a99b36f8e79b6';
            $appsecret = '3f874b3f4c9656325adeb146c9d24161';
            $data = $this->getOpenid($appid, $appsecret, $code); // 获得 openid 和 session_key

            $user = Db::Table('snipe_user')->field('user_id,openid')->where(['openid' => $data['openid']])->find();
            $num = substr(time(), -6);
            if (empty($user)) {
                $arr = [
                    'openid' => $data['openid'],
                ...
                $uid = Db::Table('snipe_user')->insertGetId($arr);
    ...
    public function getUserInfo()
    {
        ...
        $user = Db::Table('snipe_user')->where('user_id', $this->user_id)->value('user_id');
        ...
        Db::Table('snipe_user')->where('user_id', $this->user_id)->update($updata);
    ...
    public function index()
    {
        ...
        # 武器库
        $arsenal = Db::Table('snipe_config_arsenal')->field('class_id,gun_name,lv,unlock_type,unlock_coin,unlock_video,video_num,is_unlock')->select();
        ...
        # 关卡配置信息
        $snipePassInfo = Db::table('snipe_config_passInfo')->order('Pass')->select();
        ...
        # 枪的配置信息
        $snipeGunInfo = Db::table('snipe_config_gunInfo')->field('rifleNo,sniperNo')->find();
        ...
        # 军衔系统
        $data['admiralRank'] = Db::table('snipe_config_admiralRank')->field('lv,admiralRank_name,harm,probability,gem_num')->select();

        # 技能配置
        $skillInfo = Db::table('snipe_config_skill')->field('skill_name,skill_text,skill_level_name,skill_type,skill_number')->select();
        $data['skillInfo'] = $this->dataGroup($skillInfo, 'skill_type');

        # 被动技能配置
        $skillInfo = Db::table('snipe_config_skill_passive')->field('skill_name,skill_lv,skill_number,skill_harm')->select();
        $data['skillPassiveInfo'] = $skillInfo;

        # 签到列表
        $signList = Db::table('snipe_signlist')->field('day,num,type,status')->select();
        $sign = Db::table('snipe_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();
        $end = end($sign);
        if ($end['add_time'] < $this->date && count($sign) >= 7) {
            # 满一周status更新为0
            Db::table('snipe_sign')->where(['user_id' => $this->user_id])->update(['status' => 1]);
            $sign = [];
        }
        ...
        # 根据IP地址是否开启误点
        $config = Db::Table('snipe_config')->find();
    ...
    {
        $user_sign = Db::table('snipe_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();
        $end = end($user_sign);
        ...
        Db::table('snipe_sign')->insert($insert);
    ...
    {
        $ranks = Db::Table('snipe_rank_one')->field('user_id,user_name,max_pass')->select();
        $userIds = array_column($ranks, 'user_id');
        $is_rank = array_keys($userIds, $this->user_id);

        $user = Db::Table('snipe_user')->field('user_name,max_pass')->where('user_id', $this->user_id)->find();
        if (empty($is_rank)) {
            $ranking = '30+';
        } else {
            $ranking = $is_rank[0] + 1;
    ...
    public function statistics_channel()
    {
        ...
        $record_channel_id = Db::Table('snipe_record_channel')->where(['user_id' => $this->user_id, 'add_date' => $date])->value('record_channel_id');
        if (empty($record_channel_id)) {
            ...
            Db::Table('snipe_record_channel')->insert($record);
...
//        $user_id = 1;
        ...
        if (strlen($app_id) > 6) {
            $app_id = Db::Table('snipe_app')->where(['app_id' => $app_id, 'play_status' => 0])->value('id');
        }

        $is_click = Db::Table('snipe_statistics_adv')->where(['user_id' => $this->user_id, 'app_id' => $post['app_id'], 'status' => 1, 'add_time' => $date])->value('statistics_id');
        ...
        if (empty($is_click)) {
            ...
            Db::Table('snipe_statistics_adv')->insert($advData);

            if ($post['status'] == 1) {
                # 广告限量
                $restrict_id = Db::Table('snipe_restrict')->where(['app_id' => $app_id, 'add_time' => $date])->value('restrict_id');
                if (empty($restrict_id)) {
                    ...
                    Db::Table('snipe_restrict')->insert($addData);
                } else {
                    Db::Table('snipe_restrict')->where(['restrict_id' => $restrict_id])->setInc('num');
                }
            }
    ...
    public function watchVideo()
    {
        ...
        $post['timestamp'] = time();
        Db::Table('snipe_watch_video')->insert($post);
