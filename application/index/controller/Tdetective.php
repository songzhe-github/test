...
class Tdetective extends Conmmon
{
   /**
    * 获取code，返回openid
    */
   public function getCode()
   {
      if (request()->isPost()) {
         ...
         $user = Db::table('TT_Detective_user')->field('user_id,openid')->where(['openid' => $data['openid']])->find();
         if (empty($user)) {
            $num = substr(time(), -6);
            $arr = [
               'openid' => $data['openid'],
   ...
   public function index()
   {
      ...
      $DressClassArr['Sofa'] = $furnitureArr[0];
      $DressClassArr['Drink'] = $furnitureArr[1];
      ...
      # 探员配置
      $roleArray = Db::table('TT_Detective_config_role')->field('id', true)->select();
      ...
      $furnitureClassArr['Sofa'] = $furnitureArray[0];
      $furnitureClassArr['Drink'] = $furnitureArray[1];
      $furnitureClassArr['Floor'] = $furnitureArray[2];
      $furnitureClassArr['Wall'] = $furnitureArray[3];
      $data['Dress'] = $furnitureClassArr;

      # 签到列表
      $signList = Db::table('TT_Detective_signlist')->field('signlist_id', true)->select();
      $sign = Db::table('TT_Detective_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();
      $end = end($sign);
      if ($end['add_time'] < $this->date && count($sign) >= 7) {
         # 满一周status更新为0
         Db::table('TT_Detective_sign')->where(['user_id' => $this->user_id])->update(['status' => 1]);
         $sign = [];
      }
      ...
      # 排行榜 - 装扮值
      $rank_dress = Db::table('TT_Detective_rank_dress')->field('user_id,user_name,dress_value')->select();
      $userIds1 = array_column($rank_dress, 'user_id');
      $is_rank = array_keys($userIds1, $this->user_id);
      $user = Db::table('TT_Detective_user')->field('user_id,user_name,dress_value')->where(['user_id' => $this->user_id])->find();
      if (empty($is_rank)) {
         if (empty($ranking1)) {
      ...
      # 根据IP地址是否开启误点
      $config = Db::table('TT_Detective_config')->find();
...
//      # 排行榜 - 关卡
//      $rank_pass = Db::table('TT_Detective_rank_pass')->field('user_id,user_name,max_pass')->select();
//      $userIds = array_column((array)$rank_pass, 'max_pass');
//      dump($userIds);
//      $is_rank = array_keys($userIds, $this->user_id);
//      dump($is_rank);
//
...
//        dump($upData);
      Db::table('TT_Detective_user')->where(['user_id' => $this->user_id])->update($upData);
   ...
   {
      $user_sign = Db::table('TT_Detective_sign')->where(['user_id' => $this->user_id, 'status' => 0])->select();
      $end = end($user_sign);
      ...
      Db::table('TT_Detective_sign')->insert($insert);
   ...
   public function statistics_channel()
   {
      ...
      $record_channel_id = Db::Table('TT_Detective_record_channel')->where(['user_id' => $this->user_id, 'add_date' => $date])->value('record_channel_id');
      if (empty($record_channel_id)) {
         ...
         Db::Table('TT_Detective_record_channel')->insert($record);
...
//        $user_id = 1;
      ...
      if (strlen($app_id) > 6) {
         $app_id = Db::table('TT_Detective_app')->where(['app_id' => $app_id, 'play_status' => 0])->value('id');
      }

      $is_click = Db::table('TT_Detective_statistics_adv')->where(['user_id' => $this->user_id, 'app_id' => $post['app_id'], 'status' => 1, 'add_time' => $date, 'type' => $post['type']])->value('statistics_id');
      ...
      if (empty($is_click)) {
         ...
         Db::table('TT_Detective_statistics_adv')->insert($advData);

         if ($post['status'] == 1) {
            # 广告限量
            $restrict_id = Db::table('TT_Detective_restrict')->where(['app_id' => $app_id, 'add_time' => $date])->value('restrict_id');
            if (empty($restrict_id)) {
               ...
               Db::table('TT_Detective_restrict')->insert($addData);
            } else {
               Db::table('TT_Detective_restrict')->where(['restrict_id' => $restrict_id])->setInc('num');
            }
         }
   ...
   public function watchVideo()
   {
      ...
      $post['timestamp'] = time();
      Db::table('TT_Detective_watch_video')->insert($post);
   ...
   {
      $json_string = file_get_contents('json/Tdetective_config_furniture.json');
      $furniture = json_decode($json_string, true);
      foreach ($furniture as $k => $v) {
         foreach ($v as $kk => $vv) {
            if ($kk == 'furniture_name' || $kk == 'furniture_energyNum' || $kk == 'furniture_unlock_coin' || $kk == 'furniture_up_coin' || $kk == 'furniture_output_vit' || $kk == 'furniture_big_pic' || $kk == 'furniture_small_pic') {
               unset($furniture[$k][$kk]);
            }
...
//      # 装修配置信息 (json文件)
//      $furnitureInfo = Db::table('TT_Detective_config_furniture')->field('id', true)->order('furniture_id')->select();
////      dump($furnitureInfo);die;
////      $decoration = config('monster_config_decoration');
//      $furniture_json = json_encode($furnitureInfo, JSON_UNESCAPED_UNICODE);
//      file_put_contents('Tdetective_config_furniture.json', $furniture_json);
