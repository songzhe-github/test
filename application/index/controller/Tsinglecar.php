...
class Tsinglecar extends Conmmon
{
    /**
     * 获取code，返回openid
     */
    public function getcarCode()
    {
        if (request()->isPost()) {
            ...
            $appid = 'tt24af5212bb8568c7';
            $appsecret = '5a0d82449bbda4b0d0584c2e0380102fdabf24d8';
            $data = $this->getTTOpenid($appid, $appsecret, $code); // 获得 openid 和 session_key

            $user = Db::Table('TT_singlecar_user')->field('user_id,openid')->where(['openid' => $data['openid']])->find();
            if (empty($user)) {
                $arr = [
                    'openid' => $data['openid'],
                    'user_name' => '游客' . time(),
                ...
                $uid = Db::Table('TT_singlecar_user')->insertGetId($arr);
                $res['mstr'] = lock_url($data['openid'] . ',' . $uid);
                return jsonResult('请求成功', 200, $res);

            } else {
//                Db::Table('TT_singlecar_user')->where(['user_id' => $user['user_id']])->update(['end_time' => date('Y-m-d H::s')]);
    ...
    public function getUserInfo()
    {
        ...
        $user = Db::Table('TT_singlecar_user')->where(['user_id' => $this->user_id])->value('user_id');
        ...
        Db::Table('TT_singlecar_user')->where(['user_id' => $this->user_id])->update($updata);
...
//        dump(unlock_url('MGpOejV5cXZlNjVMbHJBUGQ5ckVQRFlHUURjN1M%3D'));die;
        ...
        # 用户信息
        $userInfo = Db::Table('TT_singlecar_user')
        ...
        // 剩余看视频的次数
        $video_num = Db::Table('TT_singlecar_watch_video')->where(['type' => 10, 'user_id' => $this->user_id, 'add_time' => date('Y-m-d')])->count();
        ...
        # 今日榜单
        $rank_one = Db::table('TT_singlecar_rank_one')->where('user_name', $userInfo['user_name'])->find();
        $is_rank = 0;
        if ($rank_one && $rank_one['status'] == 0) {
            Db::table('TT_singlecar_rank_one')->where('user_name', $userInfo['user_name'])->update(['status' => 1]);
            $is_rank = 1;
        }
        $rankArr = [
            'is_rank' => $is_rank,
        ...
        # 三日榜单
        $rank_three = Db::table('TT_singlecar_rank_three')->where('user_name', $userInfo['user_name'])->find();
        $is_rank = 0;
        if ($rank_three && $rank_three['status'] == 0) {
            Db::table('TT_singlecar_rank_three')->where('user_name', $userInfo['user_name'])->update(['status' => 1]);
            $is_rank = 1;
        }
        $rankArr1 = [
            'is_rank' => $is_rank,
        ...
        // 车库
        $res = Db::Table('TT_singlecar_garages_class')->where(['is_show' => 1])->select();
        $arr = Db::Table('TT_singlecar_garages')->select();
        foreach ($res as $k => $v) {
            foreach ($arr as $i => $j) {
...
//            dump($car);die;
        ...
        # 匹配
        $data['match_car'] = Db::Table('TT_singlecar_garages')->select();
        ...
        # 根据IP地址是否开启误点
        $config = Db::Table('TT_singlecar_config')->find();
    ...
    {
        $user_name = Db::table('TT_singlecar_user')->where('user_id', $this->user_id)->value('user_name');
        # 单日
        $ranks = Db::Table('TT_singlecar_rank_one')->field('user_name,avatar,max_pass')->select();
        $userIds = array_column($ranks, 'user_name');
        $is_rank = array_keys($userIds, $user_name);

        $user = Db::Table('TT_singlecar_user')->field('user_name,max_pass')->where('user_id', $this->user_id)->find();
        if (empty($is_rank)) {
            $ranking1 = '99+';
        } else {
        ...
        # 三日
        $ranks_three = Db::Table('TT_singlecar_rank_three')->field('user_name,avatar,max_pass')->select();
        $userIds = array_column($ranks, 'user_name');
        $is_rank = array_keys($userIds, $user_name);
...
//        dump($upData);
    ...
    public function challenge()
    {
        ...
        $one_length = $redis->zCard('TT_SingleCarRank_one');
        # 一日
        $one_score = $redis->zScore('TT_SingleCarRank_one', $post['user_name']);
        if ($one_score) {
            if ($post['max_pass'] > $one_score) {
                $redis->zAdd('TT_SingleCarRank_one', $post['max_pass'], $post['user_name']);
            }
        } else {
            if ($one_length < 100) {
                $redis->zAdd('TT_SingleCarRank_one', $post['max_pass'], $post['user_name']);
                ...
                // 返回分数从高到低排序的前10名及分数
                $revRange = $redis->zRange('TT_SingleCarRank_one', 0, $one_length - 1, true);
                $key = array_keys($revRange);
                $value = array_values($revRange);

                if ((int)$post['max_pass'] > (int)$value[0]) {
                    // 删除成员
                    $redis->zRem('TT_SingleCarRank_one', $key[0]);
                    $redis->zAdd('TT_SingleCarRank_one', $post['max_pass'], $post['user_name']);
                }
            }
        }

        $three_length = $redis->zCard('TT_SingleCarRank_three');
        # 一日
        $three_score = $redis->zScore('TT_SingleCarRank_three', $post['user_name']);
        if ($three_score) {
            if ($post['max_pass'] > $three_score) {
                $redis->zAdd('TT_SingleCarRank_three', $post['max_pass'], $post['user_name']);
            }
        } else {
            if ($three_length < 100) {
                $redis->zAdd('TT_SingleCarRank_three', $post['max_pass'], $post['user_name']);
                ...
                // 返回分数从高到低排序的前10名及分数
                $revRange = $redis->zRange('TT_SingleCarRank_three', 0, $three_length - 1, true);
                $key = array_keys($revRange);
                $value = array_values($revRange);

                if ((int)$post['max_pass'] > (int)$value[0]) {
                    // 删除成员
                    $redis->zRem('TT_SingleCarRank_three', $key[0]);
                    $redis->zAdd('TT_SingleCarRank_three', $post['max_pass'], $post['user_name']);
                }
            }
        }

        $rankArr = $redis->zRevRange('TT_SingleCarRank_one', 0, 4, true);
        $ranks = [];
        ...
        $count = $redis->zRevRange('TT_SingleCarRank_one', $one_length - 1, $one_length - 1, true);
        $five = $redis->zRevRange('TT_SingleCarRank_one', 0, 4, true);
        $start = array_values($five)[count(array_values($five)) - 1];
        $end = array_values($count)[count(array_values($count)) - 1];
        if ($post['max_pass'] >= $start) {
            $ranking = $redis->zRevRank('TT_SingleCarRank_one', $post['user_name']) + 1; // 从高到低排序的名次
    ...
    public function statistics_channel()
    {
        ...
        $record_channel_id = Db::Table('TT_singlecar_record_channel')->where(['user_id' => $this->user_id, 'add_date' => $date])->value('record_channel_id');
        if (empty($record_channel_id)) {
            ...
            Db::Table('TT_singlecar_record_channel')->insert($record);
    ...
    public function watchVideo()
    {
        ...
        $post['timestamp'] = time();
        Db::Table('TT_singlecar_watch_video')->insert($post);
...
//            'add_time' => date('Y-m-d H:i:s'),
//        ];
//        Db::Table('TT_singlecar_record')->insert($insert);
//        return jsonResult('用户行为记录成功', 200);
    ...
    public function freeAcceleration()
    {
        ...
        Db::Table('TT_singlecar_free_acceleration')->insert($insert);
        return jsonResult('用户行为记录成功', 200);
    }
    ...
    public function demo()
    {
        ...
        if ($one_score) {
            ...
            if ($one_length < 100) {
                ...
                // 返回分数从高到低排序的前10名及分数
                $revRange = $redis->zRevRange('TT_SingleCarRank_one', 0, $one_length - 1, true);
                $key = array_keys($revRange);
                $value = array_values($revRange);
        ...
        # 三日
        $score = $redis->zScore('TT_SingleCarRank_three', $post['user_name']);
        if ($score) {
            ...
            if ($three_length < 100) {
                ...
                // 返回分数从高到低排序的前10名及分数
                $revRange = $redis->zRevRange('TT_SingleCarRank_three', 0, $three_length - 1, true);
                $key = array_keys($revRange);
                $value = array_values($revRange);
        ...
        $end = array_values($count)[count(array_values($count)) - 1];
        if ($post['max_pass'] >= $start) {
            $ranking = $redis->zRevRank('TT_SingleCarRank_one', $post['user_name']); // 从高到低排序的名次
        } elseif ($post['max_pass'] < $start && $post['max_pass'] >= $end) {
            $ranking = rand(5, $one_length);
        } else {
            $ranking = '99+';
