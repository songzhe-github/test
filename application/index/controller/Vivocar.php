...
class Vivocar extends Conmmon
{
    public function cronTab_rank()
    {
        $insert = Db::table('vivo_car_user')->field('user_id,user_name,avatar,max_pass,now() as add_time')->where('login_date', date('Y-m-d'))->order('max_pass DESC')->limit(30)->select();
        $rank = Db::table('vivo_car_rank')->select();
//        dump($insert);die;

        if (empty($rank)) {
            Db::table('vivo_car_rank')->insertAll($insert);
        } else {
            foreach ($insert as $k => $v) {
                Db::table('vivo_car_rank')->where(['rank_id' => $k + 1])->update($v);
            }
        }
    }

    /**
     * @pkgName: com.bailimu.juedifeichesai.vivominigame 游戏包名
     * @token: 登录时获得的token
     * @timestamp: 时间戳
     * @nonce: 随机数（保证每次请求唯一性，推荐32位数字）
     * @signature: 签名 按规则生成
     */
...
//            $pkgName = input('post.pkgName');
            $pkgName = 'com.csnd.xcrkp.vivominigame';
            $timestamp = time() * 1000;
            $nonce = setrandomNumber();
            $appKey = '8ed931b4a81483c9b3f32918d353586b';
            $appSecret = 'd52d5740fcb433ae4482ce37296e146d';

            $data = $this->getVIVOOpenid($pkgName, $token, $timestamp, $nonce, $appKey, $appSecret);
            return jsonResult('succ', 200, $data);

            $user = Db::table('vivo_car_user')->field('user_id,openid')->where(['openid' => $data['openid']])->find();
            $num = substr(time(), -6);
            ...
            if (empty($user)) {
                ...
                $uid = Db::table('vivo_car_user')->insertGetId($arr);
                $res['mstr'] = lock_url($data['openid'] . ',' . $uid);
                return jsonResult('请求成功', 200, $res);

            } else {
//                Db::table('vivo_car_user')->where(['user_id' => $user['user_id']])->update(['end_time' => date('Y-m-d H::s')]);
    ...
    public function getUserInfo()
    {
        ...
        $user = Db::table('vivo_car_user')->where(['user_id' => $this->user_id])->value('user_id');
        ...
        Db::table('vivo_car_user')->where(['user_id' => $this->user_id])->update($updata);
    ...
    public function first()
    {
        ...
        # 用户信息
        $userInfo = Db::table('vivo_car_user')
        ...
        // 剩余看视频的次数
        $video_num = Db::table('vivo_car_watch_video')->where(['type' => 7, 'user_id' => $this->user_id, 'add_time' => date('Y-m-d')])->count();
        $userInfo['video_num'] = (5 - $video_num) < 0 ? 0 : 5 - $video_num;

        // 车库
        $res = Db::table('vivo_car_garages_class')->where(['is_show' => 1])->select();
        $arr = Db::table('vivo_car_garages')->select();
        foreach ($res as $k => $v) {
            foreach ($arr as $i => $j) {
...
//            dump($car);die;
        ...
        # 匹配
        $data['match_car'] = Db::table('vivo_car_garages')->select();
        ...
        # 根据IP地址是否开启误点
        $config = Db::table('vivo_car_config')->find();
    ...
    {
        $ranks = Db::table('vivo_car_rank')->field('user_id,user_name,avatar,max_pass')->select();
        $userIds = array_column($ranks, 'user_id');
        $is_rank = array_keys($userIds, $this->user_id);

        $user = Db::table('vivo_car_user')->field('user_id,user_name,avatar,max_pass')->where(['user_id' => $this->user_id])->find();
        if (empty($is_rank)) {
            if (empty($ranking)) {
//                $ranking = Db::table('vivo_car_user')->where(['max_pass' => ['>', $user['max_pass']]])->count();
...
//        }
//
//        $res = Db::table('vivo_car_challenge')->field('status')->where(['user_id' => $this->user_id, 'timestamp' => $post['timestamp']])->find();
//        if ($res) {
//            if ($res['status'] == 0) {
//                $updata = [
//                    'status' => $post['status'],
//                    'end_time' => date('Y-m-d H:i:s'),
//                ];
//                Db::table('vivo_car_challenge')->where(['user_id' => $this->user_id, 'timestamp' => $post['timestamp']])->update($updata);
...
//                'end_time' => date('Y-m-d H:i:s'),
//            ];
//            Db::table('vivo_car_challenge')->insert($insert);
...
//        }
    ...
    public function statistics_qd()
    {
        ...
        $user = Db::table('vivo_car_user')->field('user_id,channel,login_date')->where(['user_id' => $this->user_id])->find();
...
//        $user_id = 1;
        ...
        if (strlen($app_id) > 6) {
            $app_id = Db::table('vivo_car_app')->where(['app_id' => $app_id, 'play_status' => 0])->value('id');
        }

        $is_click = Db::table('vivo_car_statistics_adv')->where(['user_id' => $this->user_id, 'app_id' => $post['app_id'], 'status' => 1, 'add_time' => $date])->value('statistics_id');
        ...
        if (empty($is_click)) {
            ...
            Db::table('vivo_car_statistics_adv')->insert($advData);

            if ($post['status'] == 1) {
                # 广告限量
                $restrict_id = Db::table('vivo_car_restrict')->where(['app_id' => $app_id, 'add_time' => $date])->value('restrict_id');
                if (empty($restrict_id)) {
                    ...
                    Db::table('vivo_car_restrict')->insert($addData);
                } else {
                    Db::table('vivo_car_restrict')->where(['restrict_id' => $restrict_id])->setInc('num');
                }
            }
    ...
    public function watchVideo()
    {
        ...
        $post['timestamp'] = time();
        Db::table('vivo_car_watch_video')->insert($post);
...
//            'add_time' => date('Y-m-d H:i:s'),
//        ];
//        Db::table('vivo_car_record')->insert($insert);
